...
def get_top5(input1,user_label,item_label,item_prediction,clean_data,vectorizer,sentiment_model):
    input_user=user_label.transform([input1.lower()])[0]
    #Generate top 20 recommended products from item-based recommendation system for this user
    #Top 20 items for input_user
    pred20=pd.DataFrame(item_prediction.loc[:,input_user].sort_values(ascending=False)[0:20]).rename(columns={input_user:'predicted_sentiment'}).reset_index()
    ...
    for item in pred20.item_name:
        positive_count=0
        total_reviews=0
        for review in clean_data[clean_data.name==item].text_title_lemma:
            total_reviews+=1
            review_vec=vectorizer.transform([review])
            if sentiment_model.predict(review_vec)[0]=='Positive':
                positive_count+=1
        output_dict['item_name'].append(item)
        output_dict['total_reviews'].append(total_reviews)
        output_dict['positive_fraction'].append(round(positive_count/total_reviews,3))
